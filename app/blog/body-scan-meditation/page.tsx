...
import { bodyScanContent } from "@/lib/blog-content/body-scan";
import DownloadButtons from "@/components/DownloadButtons";

export default function BodyScanMeditationPost() {
  const { language } = useLanguage();
  const content = bodyScanContent[language];

  return (
    <article className="relative overflow-hidden">
      <div className="absolute inset-0">
        <div className="absolute top-20 right-20 w-96 h-96 bg-teal-500/10 rounded-full blur-[150px]"></div>
        <div className="absolute bottom-20 left-20 w-96 h-96 bg-cyan-500/10 rounded-full blur-[150px]"></div>
      </div>
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <header className="mb-12">
          <div className="mb-6">
            <span className="inline-block bg-gradient-to-r from-teal-500/20 to-cyan-500/20 backdrop-blur-sm px-6 py-2 rounded-full text-sm font-bold text-teal-300 border border-teal-500/30">
              {content.category}
            </span>
          </div>
        ...
        <div className="prose prose-invert prose-lg max-w-none">
          ...
          <div className="bg-gradient-to-br from-teal-500/10 to-cyan-500/10 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-teal-500/20 mb-8">
          ...
          <div className="space-y-8">
            {content.content.techniques.map((technique: any, index: number) => (
              <div key={index} className="bg-gradient-to-br from-cyan-500/10 to-transparent backdrop-blur-xl rounded-2xl p-8 border border-cyan-500/20">
          ...
          <div className="bg-gradient-to-br from-teal-500/20 to-cyan-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-teal-500/30 mt-12">
        ...
        <div className="mt-16 bg-gradient-to-br from-teal-500/10 to-cyan-500/10 backdrop-blur-xl rounded-3xl p-10 border border-teal-500/30 text-center">
        ...
        <div className="mt-16 pt-12 border-t border-white/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Link href="/blog/guided-meditation-beginners" className="group bg-gradient-to-br from-blue-500/10 to-transparent backdrop-blur-xl rounded-2xl p-6 border border-blue-500/20 hover:border-blue-500/50 transition-all duration-300">
              <h4 className="text-xl font-bold text-white mb-2 group-hover:text-blue-300 transition-colors">{content.relatedPosts.guided}</h4>
              <p className="text-gray-400 text-sm">{content.relatedPosts.guidedDesc}</p>
            </Link>

...
import { morningRoutinesContent } from "@/lib/blog-content/morning-routines";

export default function MorningRoutinesMentalHealthPost() {
  const { language } = useLanguage();
  const content = morningRoutinesContent[language];

  return (
    <article className="relative overflow-hidden">
      <div className="absolute inset-0">
        <div className="absolute top-20 right-20 w-96 h-96 bg-orange-500/10 rounded-full blur-[150px]"></div>
        <div className="absolute bottom-20 left-20 w-96 h-96 bg-amber-500/10 rounded-full blur-[150px]"></div>
      </div>
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <header className="mb-12">
          <div className="mb-6">
            <span className="inline-block bg-gradient-to-r from-orange-500/20 to-amber-500/20 backdrop-blur-sm px-6 py-2 rounded-full text-sm font-bold text-orange-300 border border-orange-500/30">
              {content.category}
            </span>
          </div>
          ...
          <div className="space-y-8">
            {content.content.benefits.map((benefit: any, index: number) => (
              <div key={index} className="bg-gradient-to-br from-orange-500/10 to-transparent backdrop-blur-xl rounded-2xl p-8 border border-orange-500/20">
                <h3 className="text-2xl font-bold text-orange-300 mb-4">{benefit.title}</h3>
        ...
        <div className="mt-16 bg-gradient-to-br from-orange-500/10 to-amber-500/10 backdrop-blur-xl rounded-3xl p-10 border border-orange-500/30 text-center">
          ...
          <a href="https://apps.apple.com/us/app/shinemind/id6754637065" target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-500 to-amber-500 text-white px-10 py-4 rounded-full text-lg font-bold hover:from-orange-600 hover:to-amber-600 transition-all transform hover:scale-105 shadow-2xl shadow-orange-500/40">

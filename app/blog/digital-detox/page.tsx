...
import { digitalDetoxContent } from "@/lib/blog-content/digital-detox";
import DownloadButtons from "@/components/DownloadButtons";

export default function DigitalDetoxPost() {
  const { language } = useLanguage();
  const content = digitalDetoxContent[language];
    ...
    <article className="relative overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <header className="mb-12">
          <div className="mb-6">
            <span className="inline-block bg-gradient-to-r from-indigo-500/20 to-blue-500/20 backdrop-blur-sm px-6 py-2 rounded-full text-sm font-bold text-indigo-300 border border-indigo-500/30">
              {content.category}
            </span>
          </div>
        ...
        <div className="prose prose-invert prose-lg max-w-none">
          ...
          <div className="bg-gradient-to-br from-indigo-500/10 to-blue-500/10 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-indigo-500/20 mb-8">
          ...
          <div className="bg-gradient-to-br from-purple-500/20 to-indigo-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-purple-500/30 mb-8">
            <h2 className="text-3xl font-bold text-white mb-6">{content.content.signs.title}</h2>
            <div className="space-y-4">
              {content.content.signs.symptoms.map((item: any, index: number) => (
                <div key={index} className="flex items-start gap-3">
                  <span className="text-purple-400 font-bold mt-1">→</span>
                  <span className="text-gray-300"><strong className="text-white">{item.sign}:</strong> {item.description}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-8">
            {content.content.strategies.map((strategy: any, index: number) => (
              <div key={index} className="bg-gradient-to-br from-blue-500/10 to-transparent backdrop-blur-xl rounded-2xl p-8 border border-blue-500/20">
                <h3 className="text-2xl font-bold text-blue-300 mb-4">{strategy.title}</h3>
                <p className="text-gray-300 leading-relaxed mb-4">{strategy.description}</p>
                {strategy.howTo && (
                  <div className="bg-white/5 rounded-xl p-4 mt-4">
                    <p className="text-sm text-gray-400 italic">{strategy.howTo}</p>
                  </div>
                )}
              </div>
            ))}
          ...
          <div className="bg-gradient-to-br from-cyan-500/20 to-blue-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-cyan-500/30 mt-12">
            ...
            <div className="space-y-4">
              {content.content.weekPlan.days.map((item: any, index: number) => (
                <div key={index} className="flex items-start gap-3">
                  <span className="text-cyan-400 font-bold mt-1 min-w-[60px]">{item.day}:</span>
          ...
          <div className="bg-gradient-to-br from-indigo-500/20 to-violet-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-indigo-500/30 mt-8">
        ...
        <div className="mt-16 bg-gradient-to-br from-indigo-500/10 to-blue-500/10 backdrop-blur-xl rounded-3xl p-10 border border-indigo-500/30 text-center">

...
import { meditationBenefitsContent } from "@/lib/blog-content/meditation-benefits";

export default function BenefitsOfDailyMeditationPost() {
  const { language } = useLanguage();
  const content = meditationBenefitsContent[language];
          ...
          <div className="space-y-8">
            {content.content.benefits.map((benefit: any, index: number) => (
              <div key={index} className="bg-gradient-to-br from-blue-500/10 to-transparent backdrop-blur-xl rounded-2xl p-8 border border-blue-500/20">
                <h3 className="text-2xl font-bold text-blue-300 mb-4">{benefit.title}</h3>
                <p className="text-gray-300 leading-relaxed">{benefit.description}</p>
              </div>
            ))}
          </div>

          <div className="bg-gradient-to-br from-purple-500/20 to-pink-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-purple-500/30 mt-12">

...
import { improveFocusContent } from "@/lib/blog-content/improve-focus-concentration";
import DownloadButtons from "@/components/DownloadButtons";

export default function ImproveFocusPost() {
  const { language } = useLanguage();
  const content = improveFocusContent[language];

  return (
    <article className="relative overflow-hidden">
      <div className="absolute inset-0">
        <div className="absolute top-20 right-20 w-96 h-96 bg-sky-500/10 rounded-full blur-[150px]"></div>
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <header className="mb-12">
          <div className="mb-6">
            <span className="inline-block bg-gradient-to-r from-sky-500/20 to-blue-500/20 backdrop-blur-sm px-6 py-2 rounded-full text-sm font-bold text-sky-300 border border-sky-500/30">
              {content.category}
            </span>
          </div>
        ...
        <div className="prose prose-invert prose-lg max-w-none">
          ...
          <div className="bg-gradient-to-br from-sky-500/10 to-blue-500/10 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-sky-500/20 mb-8">
          ...
          <div className="space-y-8">
            {content.content.techniques.map((technique: any, index: number) => (
              <div key={index} className="bg-gradient-to-br from-blue-500/10 to-transparent backdrop-blur-xl rounded-2xl p-8 border border-blue-500/20">
                <h3 className="text-2xl font-bold text-sky-300 mb-4">{technique.title}</h3>
          ...
          <div className="bg-gradient-to-br from-sky-500/20 to-blue-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-sky-500/30 mt-12">
        ...
        <div className="mt-16 bg-gradient-to-br from-sky-500/10 to-blue-500/10 backdrop-blur-xl rounded-3xl p-10 border border-sky-500/30 text-center">
        ...
        <div className="mt-16 pt-12 border-t border-white/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Link href="/blog/overcoming-procrastination" className="group bg-gradient-to-br from-red-500/10 to-transparent backdrop-blur-xl rounded-2xl p-6 border border-red-500/20 hover:border-red-500/50 transition-all duration-300">
              <h4 className="text-xl font-bold text-white mb-2 group-hover:text-red-300 transition-colors">{content.relatedPosts.procrastination}</h4>
              <p className="text-gray-400 text-sm">{content.relatedPosts.procrastinationDesc}</p>
            </Link>

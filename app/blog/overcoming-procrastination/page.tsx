...
import { overcomingProcrastinationContent } from "@/lib/blog-content/overcoming-procrastination";
import DownloadButtons from "@/components/DownloadButtons";

export default function OvercomingProcrastinationPost() {
  const { language } = useLanguage();
  const content = overcomingProcrastinationContent[language];
    ...
    <article className="relative overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <header className="mb-12">
          <div className="mb-6">
            <span className="inline-block bg-gradient-to-r from-red-500/20 to-orange-500/20 backdrop-blur-sm px-6 py-2 rounded-full text-sm font-bold text-red-300 border border-red-500/30">
              {content.category}
            </span>
          </div>
        ...
        <div className="prose prose-invert prose-lg max-w-none">
          ...
          <div className="bg-gradient-to-br from-red-500/10 to-orange-500/10 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-red-500/20 mb-8">
          ...
          <div className="space-y-8">
            {content.content.techniques.map((technique: any, index: number) => (
              <div key={index} className="bg-gradient-to-br from-orange-500/10 to-transparent backdrop-blur-xl rounded-2xl p-8 border border-orange-500/20">
                <h3 className="text-2xl font-bold text-orange-300 mb-4">{technique.title}</h3>
          ...
          <div className="bg-gradient-to-br from-amber-500/20 to-orange-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-amber-500/30 mt-12">
            <h2 className="text-3xl font-bold text-white mb-6">{content.content.rootCauses.title}</h2>
            <div className="space-y-4">
              {content.content.rootCauses.triggers.map((item: any, index: number) => (
                <div key={index} className="flex items-start gap-3">
                  <span className="text-amber-400 font-bold mt-1">→</span>
                  <span className="text-gray-300"><strong className="text-white">{item.trigger}:</strong> {item.solution}</span>
                </div>
              ))}
            </div>
          </div>

          <div className="bg-gradient-to-br from-red-500/20 to-rose-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-red-500/30 mt-8">
        ...
        <div className="mt-16 bg-gradient-to-br from-red-500/10 to-orange-500/10 backdrop-blur-xl rounded-3xl p-10 border border-red-500/30 text-center">
        ...
        <div className="mt-16 pt-12 border-t border-white/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Link href="/blog/mindfulness-busy-professionals" className="group bg-gradient-to-br from-teal-500/10 to-transparent backdrop-blur-xl rounded-2xl p-6 border border-teal-500/20 hover:border-teal-500/50 transition-all duration-300">
              <h4 className="text-xl font-bold text-white mb-2 group-hover:text-teal-300 transition-colors">{content.relatedPosts.mindfulness}</h4>
              <p className="text-gray-400 text-sm">{content.relatedPosts.mindfulnessDesc}</p>
            </Link>
          </div>
        </div>
      </div>

...
import { breathingExercisesContent } from "@/lib/blog-content/breathing-exercises";
import DownloadButtons from "@/components/DownloadButtons";

export default function BreathingExercisesPost() {
  const { language } = useLanguage();
  const content = breathingExercisesContent[language];

  return (
    <article className="relative overflow-hidden">
      <div className="absolute inset-0">
        <div className="absolute top-20 right-20 w-96 h-96 bg-cyan-500/10 rounded-full blur-[150px]"></div>
        <div className="absolute bottom-20 left-20 w-96 h-96 bg-teal-500/10 rounded-full blur-[150px]"></div>
      </div>
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <header className="mb-12">
          <div className="mb-6">
            <span className="inline-block bg-gradient-to-r from-cyan-500/20 to-teal-500/20 backdrop-blur-sm px-6 py-2 rounded-full text-sm font-bold text-cyan-300 border border-cyan-500/30">
              {content.category}
            </span>
          </div>
        ...
        <div className="prose prose-invert prose-lg max-w-none">
          ...
          <div className="bg-gradient-to-br from-cyan-500/10 to-teal-500/10 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-cyan-500/20 mb-8">
          ...
          <div className="space-y-8">
            {content.content.techniques.map((technique: any, index: number) => (
              <div key={index} className="bg-gradient-to-br from-teal-500/10 to-transparent backdrop-blur-xl rounded-2xl p-8 border border-teal-500/20">
                <h3 className="text-2xl font-bold text-teal-300 mb-4">{technique.title}</h3>
          ...
          <div className="bg-gradient-to-br from-cyan-500/20 to-teal-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-cyan-500/30 mt-12">
            <h2 className="text-3xl font-bold text-white mb-6">{content.content.whenToUse.title}</h2>
            <div className="space-y-4">
              {content.content.whenToUse.situations.map((item: any, index: number) => (
                <div key={index} className="flex items-start gap-3">
                  <span className="text-cyan-400 font-bold mt-1">→</span>
                  <span className="text-gray-300"><strong className="text-white">{item.scenario}:</strong> {item.technique} — <em className="text-gray-400">{item.reason}</em></span>
                </div>
              ))}
            </div>
          </div>

          <div className="bg-gradient-to-br from-teal-500/20 to-cyan-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-teal-500/30 mt-8">
        ...
        <div className="mt-16 bg-gradient-to-br from-cyan-500/10 to-teal-500/10 backdrop-blur-xl rounded-3xl p-10 border border-cyan-500/30 text-center">

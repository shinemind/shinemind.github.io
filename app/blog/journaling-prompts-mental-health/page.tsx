...
import { journalingPromptsContent } from "@/lib/blog-content/journaling-prompts";
import DownloadButtons from "@/components/DownloadButtons";

export default function JournalingPromptsPost() {
  const { language } = useLanguage();
  const content = journalingPromptsContent[language];
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          ...
          <div className="space-y-8">
            {content.content.techniques.map((technique: any, index: number) => (
              <div key={index} className="bg-gradient-to-br from-yellow-500/10 to-transparent backdrop-blur-xl rounded-2xl p-8 border border-yellow-500/20">
                ...
                <p className="text-gray-300 leading-relaxed mb-6">{technique.description}</p>
                {technique.prompts && (
                  <div className="bg-white/5 rounded-xl p-6">
                    <ul className="space-y-3">
                      {technique.prompts.map((prompt: string, promptIndex: number) => (
                        <li key={promptIndex} className="flex items-start gap-3">
                          <span className="text-amber-400 font-bold mt-0.5 shrink-0">{promptIndex + 1}.</span>
                          <span className="text-gray-300 text-sm leading-relaxed">{prompt}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
        ...
        <div className="mt-16 pt-12 border-t border-white/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Link href="/blog/how-to-start-gratitude-journal" className="group bg-gradient-to-br from-amber-500/10 to-transparent backdrop-blur-xl rounded-2xl p-6 border border-amber-500/20 hover:border-amber-500/50 transition-all duration-300">
              <h4 className="text-xl font-bold text-white mb-2 group-hover:text-amber-300 transition-colors">{content.relatedPosts.gratitude}</h4>

...
import { sleepMeditationContent } from "@/lib/blog-content/sleep-meditation";
import DownloadButtons from "@/components/DownloadButtons";

export default function SleepMeditationPost() {
  const { language } = useLanguage();
  const content = sleepMeditationContent[language];
        ...
        <div className="prose prose-invert prose-lg max-w-none">
          ...
          <div className="space-y-6 mb-12">
            {content.content.benefits.map((benefit: any, index: number) => (
              <div key={index} className="bg-gradient-to-br from-violet-500/10 to-transparent backdrop-blur-xl rounded-2xl p-6 border border-violet-500/20">
                <h3 className="text-xl font-bold text-violet-300 mb-2">{benefit.title}</h3>
          ...
          <div className="space-y-8">
            {content.content.techniques.map((technique: any, index: number) => (
              <div key={index} className="bg-gradient-to-br from-indigo-500/10 to-transparent backdrop-blur-xl rounded-2xl p-8 border border-indigo-500/20">
          ...
          <div className="bg-gradient-to-br from-indigo-500/20 to-violet-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-indigo-500/30 mt-12">
            <h2 className="text-3xl font-bold text-white mb-6">{content.content.bestPractices.title}</h2>
            <ul className="space-y-4 text-gray-300">
              {content.content.bestPractices.tips.map((tip: any, index: number) => (
          ...
          <div className="bg-gradient-to-br from-violet-500/20 to-indigo-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-violet-500/30 mt-8">
            <h2 className="text-3xl font-bold text-white mb-6">{content.content.commonMistakes.title}</h2>
            <div className="space-y-4">
              {content.content.commonMistakes.mistakes.map((item: any, index: number) => (
                <div key={index} className="flex items-start gap-3">
                  <span className="text-violet-400 font-bold mt-1">✗</span>
                  <span className="text-gray-300"><strong className="text-white">{item.error}:</strong> {item.solution}</span>
                </div>
              ))}
            </div>
          </div>

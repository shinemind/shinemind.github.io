...
import { socialConnectionContent } from "@/lib/blog-content/social-connection";
import DownloadButtons from "@/components/DownloadButtons";

export default function SocialConnectionPost() {
  const { language } = useLanguage();
  const content = socialConnectionContent[language];
    ...
    <article className="relative overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <header className="mb-12">
          <div className="mb-6">
            <span className="inline-block bg-gradient-to-r from-rose-500/20 to-emerald-500/20 backdrop-blur-sm px-6 py-2 rounded-full text-sm font-bold text-rose-300 border border-rose-500/30">
              {content.category}
            </span>
          </div>
        ...
        <div className="prose prose-invert prose-lg max-w-none">
          ...
          <div className="bg-gradient-to-br from-rose-500/10 to-emerald-500/10 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-rose-500/20 mb-8">
          ...
          <div className="space-y-8">
            {content.content.techniques.map((technique: any, index: number) => (
              <div key={index} className="bg-gradient-to-br from-emerald-500/10 to-transparent backdrop-blur-xl rounded-2xl p-8 border border-emerald-500/20">
                <h3 className="text-2xl font-bold text-rose-300 mb-4">{technique.title}</h3>
          ...
          <div className="bg-gradient-to-br from-rose-500/20 to-emerald-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-rose-500/30 mt-12">
        ...
        <div className="mt-16 bg-gradient-to-br from-rose-500/10 to-emerald-500/10 backdrop-blur-xl rounded-3xl p-10 border border-rose-500/30 text-center">
        ...
        <div className="mt-16 pt-12 border-t border-white/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Link href="/blog/emotional-intelligence" className="group bg-gradient-to-br from-violet-500/10 to-transparent backdrop-blur-xl rounded-2xl p-6 border border-violet-500/20 hover:border-violet-500/50 transition-all duration-300">
              <h4 className="text-xl font-bold text-white mb-2 group-hover:text-violet-300 transition-colors">{content.relatedPosts.ei}</h4>
              <p className="text-gray-400 text-sm">{content.relatedPosts.eiDesc}</p>
            </Link>
          </div>
        </div>
      </div>

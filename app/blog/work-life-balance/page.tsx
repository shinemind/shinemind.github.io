...
import { workLifeBalanceContent } from "@/lib/blog-content/work-life-balance";
import DownloadButtons from "@/components/DownloadButtons";

export default function WorkLifeBalancePost() {
  const { language } = useLanguage();
  const content = workLifeBalanceContent[language];
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="prose prose-invert prose-lg max-w-none">
          ...
          <div className="bg-gradient-to-br from-green-500/20 to-emerald-500/20 backdrop-blur-xl rounded-3xl p-8 md:p-12 border border-green-500/30 mt-12">
            <h2 className="text-3xl font-bold text-white mb-6">{content.content.buildingBalance.title}</h2>
            <ul className="space-y-4 text-gray-300">
              {content.content.buildingBalance.strategies.map((strategy: any, index: number) => (
        ...
        <div className="mt-16 pt-12 border-t border-white/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Link href="/blog/mindfulness-busy-professionals" className="group bg-gradient-to-br from-emerald-500/10 to-transparent backdrop-blur-xl rounded-2xl p-6 border border-emerald-500/20 hover:border-emerald-500/50 transition-all duration-300">
              <h4 className="text-xl font-bold text-white mb-2 group-hover:text-emerald-300 transition-colors">{content.relatedPosts.mindfulness}</h4>
